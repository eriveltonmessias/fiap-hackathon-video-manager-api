...
value class OriginalFilename private constructor(val value: String) {
	companion object {
		fun of(value: String): OriginalFilename {
			...
			require('/' !in normalized && '\\' !in normalized) { "originalFilename must not contain path separators" }
			require(normalized.none(Char::isISOControl)) { "originalFilename must not contain control characters" }
